...
# Atualizado: Correção para a Sala 01 com Y = 2.5
...
match menu:
    ...
    case "🛣️ 3. Trajetória e Colisão":
        ...
        if len(df) < 2:
            ...
            if dist_colisao <= obs['Raio']:
                st.error(f"🚨 COLISÃO DETETADA! A distância ({dist_colisao:.3f}m) é menor que o raio ({obs['Raio']}m).")
                st.warning("Planeamento de Rota de Desvio Necessário:")
                
                od1, od2 = st.columns(2)
                with od1:
                    desvio_x = st.number_input("Coordenada X do Desvio (D)", value=float(obs['X']))
                with od2:
                    desvio_y = st.number_input("Coordenada Y do Desvio (D)", value=float(obs['Y'] - obs['Raio'] - 0.5))
                
                fig.add_trace(go.Scatter(x=[x1, x2], y=[y1, y2], mode='lines', name='Trajetória Original (Bloqueada)', line=dict(color='gray', dash='dot')))
                fig.add_trace(go.Scatter(x=[x1, desvio_x, x2], y=[y1, desvio_y, y2], mode='lines+markers', name='Rota de Desvio Segura', line=dict(color='green', width=3)))
                fig.add_trace(go.Scatter(x=[desvio_x], y=[desvio_y], mode='markers+text', text=['Ponto de Desvio (D)'], textposition="bottom center", marker=dict(color='green', symbol='star', size=12), name='Desvio'))
                
                tem_desvio = True
            else:
                st.success("✅ Rota Segura! Nenhuma colisão detetada.")
            ...
            estado_traj = {'x1':x1, 'y1':y1, 'x2':x2, 'y2':y2, 'm':m, 'b':b, 'dist':dist_colisao, 'tem_desvio': tem_desvio}
            if tem_desvio:
                estado_traj['desvio_x'] = desvio_x
                estado_traj['desvio_y'] = desvio_y
            st.session_state['trajetoria'] = estado_traj

    case "📐 4. Perfil do Piso":
        ...
        try:
            ...
            st.markdown(f"**Função:** $y = {a:.2f}x^2 + {b:.2f}x + {c:.2f}$")
    ...
    case "🎯 5. Otimização Logística":
        ...
        if soma_pesos > 0:
            ...
            # Análise textual de acordo com o Novo PDF
            st.info(f"💡 **Conclusão:** Este resultado é extremamente coerente com a disposição do corredor. A coordenada X={x_c:.2f} posiciona a base numa zona intermédia (puxada ligeiramente para o lado esquerdo devido ao elevado peso da Entrada Principal), enquanto a coordenada Y={y_c:.2f} a coloca mais próxima da parede superior do corredor (onde se alinham a Sala de Aula 01 e a Biblioteca em Y=2.5).")
    ...
    case "📄 RELATÓRIO FINAL":
        ...
        with st.expander("📌 1. Georreferenciamento (Mapeamento Base)"):
        ...
        if 'trajetoria' in st.session_state:
            ...
            col1, col2 = st.columns(2)
            col1.metric("Distância Reta-Obstáculo", f"{tj['dist']:.3f} m", "- Perigo de Colisão" if tj.get('tem_desvio', False) else "Seguro", delta_color="inverse")
            col2.metric("Raio de Tolerância do Objeto", f"{obs['Raio']:.2f} m")
            
            st.write(f"- **Equação da Reta Ideal:** `y = {tj['m']:.4f}x + {tj['b']:.2f}`")
            
            if tj.get('tem_desvio', False):
                st.error("Status de Segurança: ALERTA DE COLISÃO. Rota original intercetada pelo obstáculo.")
                st.info(f"🛣️ **Ação Tomada:** Nova rota de desvio estabelecida contornando pelo ponto **D({tj['desvio_x']}, {tj['desvio_y']})**.")
            else:
        ...
        if 'parabola' in st.session_state:
            ...
            col3.metric("Pico do Desnível (Altura)", f"{p['yv']:.2f} cm")
            col4.metric("Posição Crítica (Eixo X)", f"{p['xv']:.2f} m")
        ...
        # Conclusão atualizada com base no novo PDF
        st.info(f"💡 **Análise:** Este resultado é extremamente coerente com a disposição do corredor. A coordenada X={xc:.2f} posiciona a base numa zona intermédia (puxada ligeiramente para o lado esquerdo devido ao elevado peso da Entrada Principal), enquanto a coordenada Y={yc:.2f} a coloca mais próxima da parede superior do corredor (onde se alinham a Sala de Aula 01 e a Biblioteca em Y=2.5).")
        
        st.markdown("---")
        
        # Botão de Exportação
        resumo_exportacao = f"""RELATÓRIO DE OTIMIZAÇÃO DE AGV - FACULDADE MULTIVIX
=====================================================
Coordenada Ótima da Base Operacional: ({xc:.2f}, {yc:.2f})
Grupo: Cezar Augusto, Guilherme Mello, João Vitor Vargas, Lucas Bayerl, Pedro Samuel Vieira
Disciplina: Geometria Analítica e Álgebra Linear
"""
        st.download_button(
            label="📥 Download Resumo",
            data=resumo_exportacao,
            file_name="resumo_agv_multivix.txt",
            mime="text/plain"
        )
